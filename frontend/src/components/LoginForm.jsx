...
async function apiRegister({ username, email, password, role }) {
  await delay(700);
  if (
    Object.values(DB).find((u) => u.email.toLowerCase() === email.toLowerCase())
  )
    throw new Error("Email already registered");
  if (password.length < 8)
    throw new Error("Password must be at least 8 characters");
  const trimmedUsername = username.trim();
  if (!trimmedUsername) throw new Error("Username must not be blank");
  ...
  const token = btoa(JSON.stringify({ id, role: DB[id].role }));
  const { password: _pw, ...safeUser } = DB[id];
  return { access_token: token, user: safeUser };
}
...
//  Role permissions (mirrors backend ROLE_PERMISSIONS)
const ROLE_PERMS = {
...
function ProfilePage() {
  ...
  const perms = ROLE_PERMS[displayUser?.role] || [];
  const badgeClass = `badge badge-${displayUser?.role}`;
          ...
          <>
            <div className="profile-header">
              ...
              <div>
                ...
                <span className={badgeClass}>⬡ {displayUser.role}</span>
              </div>
            </div>
            ...
            <div className="info-grid">
              ...
              <div className="info-row">
                ...
                <span className="info-val">{displayUser.role}</span>
              </div>
            </div>
